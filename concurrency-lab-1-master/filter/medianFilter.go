...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		for i := 0; i < threads; i++ {
			startY := i * height / threads
			endY := (i + 1) * height / threads

			go worker(startY, endY, 0, width, immutableData, channels[i])
		}

		parts := make([][][]uint8, threads)
		for i := 0; i < len(channels); i++ {
			parts[i] = <-channels[i]
		}

		for i := 0; i < len(parts); i++ {
			newPixelData = append(newPixelData, parts[i]...)
		}
